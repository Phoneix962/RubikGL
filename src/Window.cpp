...
void Window::draw_algorithm_frame()
{
	ImGuiWindowFlags window_flags = 0;
	window_flags |= ImGuiWindowFlags_NoResize;
	window_flags |= ImGuiWindowFlags_NoCollapse;
	static char algInput[256] = "";
	ImGui::Begin("Algorithm Input", NULL, ImGuiWindowFlags_AlwaysAutoResize);
	ImGui::InputText("Algorithm", algInput, IM_ARRAYSIZE(algInput));

	if (ImGui::Button("Execute"))
	{
		cube->algInput(std::string(algInput)); // Fixed by explicitly calling the member function  
		algInput[0] = '\0'; // clear input  
	}
	ImGui::End();
}
...
void Window::processInput(int key, int scancode, int action, int mods)
{
	...
	if ((!viewRotating && action == GLFW_PRESS) && !(io->WantTextInput))
	{
		RotateDirection dir;
		int faceIndex = 0;
